...
app.post("/api/quotes", async (req, res, next) => {
  try {
    const { text, author } = req.body; // get the request info

    const newQuote = { text, author }; // create variable save data

    await Quote.create(newQuote); //tell module to create info
...
app.delete("/api/quotes/:id", async (req, res, next) => {
  try {
    //DELETE /api/quotes/:id uses req.params.id to get the id
    const deleteId = Number(req.params.id);
    const deleteQuote = await Quote.findByPk(deleteId);
    if (!deleteQuote) {
      res.sendStatus(404);
    }
    deleteQuote.destroy();
